...
export class Connection extends EventEmitter {
  ...
  completeConfiguration(): Promise<any> {
    ...
    for (let queueId in this._queues) {
      let queue: Queue = this._queues[queueId];
    ...
    for (let bindingId in this._bindings) {
      let binding: Binding = this._bindings[bindingId];
...
export class Message {
 ...
 async sendTo(destination: Exchange | Queue, routingKey: string = ""): Promise<any> {
    ...
    if (await destination.initialized) {
      sendMessage();
    } else {
...
export class Queue {
 ...
 async publish(content: any, options: any = {}): Promise<any> {
   ...
   */
 async startConsumer(onMessage: (msg: any, channel?: AmqpLib.Channel) => any,
                options: Queue.StartConsumerOptions = {})
      : Promise<Queue.StartConsumerResult> {
    if (await this._consumerInitialized) {
 ...
 async activateConsumer(onMessage: (msg: Message) => any,
  ...
  _initializeConsumer(): void {
    let processedMsgConsumer = (msg: AmqpLib.Message) => {
      try {
        /* istanbul ignore if */
        if (!msg) {
    ...
    let rawMsgConsumer = (msg: AmqpLib.Message) => {
      try {
    ...
    let activateConsumerWrapper = (msg: AmqpLib.Message) => {
      try {
...
export class Binding {
  ...
  _initialize(): void {
    this.initialized = new Promise<Binding>((resolve, reject) => {
      if (this._destination instanceof Queue) {
        let queue = <Queue>this._destination;
        ...
        let exchange = <Exchange>this._destination;
  ...
  static removeBindingsContaining(connectionPoint: Exchange | Queue): Promise<any> {
    ...
    for (let bindingId in connection._bindings) {
      ...
      let binding: Binding = connection._bindings[bindingId];
